...
import { panelPixelsToPng, svgToPanelPixels } from '../../common/utils/panel-image.util';
...
export class SetupScreenService implements OnModuleInit {
  ...
  async generateSetupScreen(
    ...
    try {
      ...
      // Always 1-bit: this screen is shown at provisioning, before a device is linked to a model,
      // and black text on white reads correctly on every panel depth.
      const panel = await svgToPanelPixels(svg, 2);
      await fs.writeFile(this.setupScreenPath, await panelPixelsToPng(panel));
